...
for deg=0:2:noRev*360 % Revolutional angle 공전각도 (2 Revolutions)
    clf;
    % ex3에서 추가된 부분:
    % gaussian distribution을 따르는 1x1 랜덤수를 생성하여 각 평행이동 좌표에 더해준다.
    % (x1, x2에 랜덤수를 더하지 않고 x3에만 더하면 하강할 때만 랜덤하게 떨리고,
    %     x3축을 마주보고 x1x2 평면을 관찰했을 때 예쁜 원이 그려진다.
    x1=r_Rev*cos(deg*pi/180);
    x2=r_Rev*sin(deg*pi/180);
    % 하강할 때 좀 더 격하게 떨리도록 해보려고 랜덤수에 5를 곱해주었다.
    x3=-deg/(noRev*360)*20+5*randn(1);
