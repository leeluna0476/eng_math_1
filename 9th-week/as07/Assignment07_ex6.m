...
vgain = 5000; % 원근감 조절을 위한 사용자 설정 계수
...
for x1=0:0.15:20
    ...
    % viewpoint가 물체의 자전보다 2배 느리게 x3축 기준으로 반지름 60의 원운동을 하도록 한다.
    % 관찰자가 x3축 기준으로 1바퀴 공전하는 셈.
    % 다음 루틴에서 찍는 그림과 매끄럽게 연결되게 하기 위해 (viewpoint가 갑자기 확 바뀌지 않게..)
    %     정확히 원래 위치로 돌아오도록 설정했다.
    % noRot1보다 큰 수로 나누면 1바퀴보다 덜 돌아서 다음 그림과 매끄럽게 연결되지 않는다.
    vpoint(1)=60*cos(psi/2);
    vpoint(2)=60*sin(psi/2);
    ...
    % viewpoint가 어떻게 변하는지 잘 보기 위해 궤적을 그려보았다.
    vorbit = [vorbit; vpoint(1) vpoint(2) vpoint(3)];
    line(vorbit(:,1), vorbit(:,2), vorbit(:,3), 'Color', 'r');
    ...
    view(vpoint); grid;
    text(vpoint(1), vpoint(2), vpoint(3), '이서진 202500062');
    pause(0.000001);
end
